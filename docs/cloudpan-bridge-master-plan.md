...
- 以 `FTP` 作为当前第六个已实现的可写目标端
- 以 `SFTP` 作为当前第七个已实现的可写目标端
...
- `Target Adapter` 已抽出 Guangya 适配层，并补上 OpenList / LocalFS / WebDAV / S3 / FTP / SFTP 可写目标端
...
- `Phase 4`
  ...
  - OpenList / LocalFS / WebDAV / S3 / FTP / SFTP 已具备真实可写目标端适配器
